...
<div class="page-header mb-5">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-2">
            ...
            <li class="breadcrumb-item active">Modifier livraison #<?= $livraison['id'] ?></li>
        </ol>
    </nav>
    <h1 class="mb-0" style="font-weight: 800; font-size: 2.5rem; color: #1a1d29;">
        <i class="fas fa-edit me-3" style="color: #667eea;"></i>Modifier la Livraison
    </h1>
    <p class="text-muted mt-2 mb-0">Mettez à jour les informations de la livraison #<?= $livraison['id'] ?></p>
</div>
    ...
    <div class="col-lg-8">
        ...
        <?php if(count($colis) == 0): ?>
            <div class="alert alert-warning p-4" style="border-radius: 20px; font-weight: 600;">
                ...
                Aucun colis disponible. Veuillez ajouter des colis avant de modifier une livraison.
            </div>
        <?php else: ?>
            <div class="card border-0 shadow-sm" style="border-radius: 20px; overflow: hidden;">
                ...
                <div class="card-body p-4">
                    <form action="<?= BASE_URL ?>livraisons/update/<?= $livraison['id'] ?>" method="POST" id="livraisonForm">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" 
                                    ...
                                    <?php foreach ($colis as $coli): ?>
                                        <option value="<?= $coli["id"] ?>" <?= ($livraison['colis_id'] == $coli["id"]) ? 'selected' : '' ?>>
                                            Colis N°<?= $coli["id"] ?>
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" 
                                    ...
                                    <?php foreach ($chauffeurs as $chauffeur): ?>
                                        <option value="<?= $chauffeur["id"] ?>" <?= ($livraison['chauffeur_id'] == $chauffeur["id"]) ? 'selected' : '' ?>>
                                            <?= $chauffeur["nom"] ?> (ID : <?= $chauffeur["id"] ?>)
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" 
                                    ...
                                    <?php foreach ($vehicules as $vehicule): ?>
                                        <option value="<?= $vehicule["id"] ?>" <?= ($livraison['vehicule_id'] == $vehicule["id"]) ? 'selected' : '' ?>>
                                            <?= $vehicule["immatriculation"] ?> (ID : <?= $vehicule["id"] ?>)
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" 
                                    ...
                                    <?php foreach ($zones as $zone): ?>
                                        <option value="<?= $zone["id"] ?>" <?= ($livraison['zone_id'] == $zone["id"]) ? 'selected' : '' ?>>
                                            <?= $zone["nom"] ?> (ID : <?= $zone["id"] ?>)
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" 
                                    ...
                                    <?php foreach ($entrepots as $entrepot): ?>
                                        <option value="<?= $entrepot["id"] ?>" <?= ($livraison['entrepot_id'] == $entrepot["id"]) ? 'selected' : '' ?>>
                                            <?= $entrepot["nom"] ?> (adresse : <?= $entrepot["adresse"] ?>)
                                        </option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        ...
                        <div class="form-section mb-4">
                            <div class="mb-3">
                                ...
                                <input type="datetime-local" 
                                       ...
                                       id="date_livraison" 
                                       name="date_livraison"
                                       value="<?= date('Y-m-d\TH:i', strtotime($livraison['date_livraison'])) ?>"
                                       required
                                       style="border-radius: 12px; border: 2px solid #e2e8f0; padding: 0.5rem 0.75rem;">
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" 
                                    ...
                                    ?>
                                        <option value="<?= $s ?>" <?= ($livraison['statut'] === $s) ? 'selected' : '' ?>><?= ucfirst($s) ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="d-flex gap-2 justify-content-end mt-4">
                            ...
                            <button type="submit" class="btn" style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; border: none; border-radius: 12px; font-weight: 600; padding: 0.5rem 1rem;">
                                <i class="fas fa-check-circle me-2"></i>Enregistrer la modification
                            </button>
                        </div>
...
<script src="<?= BASE_URL ?>assets/js/livraisons/update.js"></script>
